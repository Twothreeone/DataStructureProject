...
//
//  Created by Charlesworth, Benjamin on 3/23/18.
...
void GraphTester :: compareTraversals()
{
    ...
    {
        cout << "DEPTH TRAVERSAL AT INDEX " << i << endl << endl;
        puzzle.depthFirstTraversal(puzzle, i);
        cout << endl;
    }
    for (int i = 0; i < puzzle.size(); i++)
    {
        cout << "BREADTH TRAVERSAL AT INDEX " << i << endl << endl;
...
void GraphTester :: findCheapestTraversal()
{
    ...
    {
        current = puzzle.costTraversal(puzzle, i);
        if (current < min && current >= 0)
        {
            min = current;
            minIndex = i;
        }
        if (current > max && current <= 500)
        {
            max = current;
            maxIndex = i;
        }
    }
    cout << "The least expensive traversal is at index " << minIndex << ", has a cost of " << min << ", and the text associated with it is:" << endl;
    puzzle.depthFirstTraversal(puzzle, minIndex);
    cout << endl;
    cout << "The most expensive traversal is at index " << maxIndex << ", has a cost of " << max << ", and the text associated with it is:" << endl;
    puzzle.depthFirstTraversal(puzzle, maxIndex);
}
